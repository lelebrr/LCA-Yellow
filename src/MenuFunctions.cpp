...
void MenuFunctions::updateStatusBar()
{
  ...
  #ifdef HAS_PAJ7620_GESTURE
    extern GestureInterface gesture_obj;
    Gesture gesture = gesture_obj.getGesture();
    if (gesture != GES_NONE) {
      display_obj.tft.setTextColor(TFT_MAGENTA, STATUSBAR_COLOR);
      #ifdef HAS_ST7996
        display_obj.tft.drawString(gesture_obj.gestureToString(gesture), 240, 0, 2);
      #else
        display_obj.tft.drawString(gesture_obj.gestureToString(gesture), 180, 0, 2);
      #endif
    }
  #endif
    }
  ...
  #ifdef HAS_DS18B20_TEMP
    extern TempInterface temp_obj;
    float current_temp = temp_obj.getTempC();
    if (current_temp > -127.0) {
      display_obj.tft.setTextColor(TFT_YELLOW, STATUSBAR_COLOR);
      #ifdef HAS_ST7796
        display_obj.tft.drawString((String)current_temp + "C", 180, 0, 2);
      #else
        display_obj.tft.drawString((String)current_temp + "C", 130, 0, 2);
      #endif
    }
  #endif
}
...
void MenuFunctions::RunSetup()
{
  ...
  gpsMenu.name = "Info GPS";
  wifiSnifferMenu.name = text_table1[20];
  wifiAttackMenu.name = text_table1[21];
  wifiLanAttackMenu.name = "Ataques WiFi & LAN";
  wifiWPSAttackMenu.name = "Ataques WPS";
  wifiRouterAttackMenu.name = "Ataques a Roteadores";
  ...
  this->addNodes(&wifiAttackMenu, "Ataques WiFi & LAN", TFTRED, NULL, ATTACKS, [this]() {
    this->changeMenu(&wifiLanAttackMenu);
  });
  ...
  this->addNodes(&wifiLanAttackMenu, "Ataques WPS", TFTORANGE, NULL, ATTACKS, [this]() {
    this->changeMenu(&wifiWPSAttackMenu);
  });
  ...
  #ifdef HAS_GPS
    this->addNodes(&deviceMenu, "Info GPS", TFTGREEN, NULL, GPS_MENU, [this]() {
...
#ifdef HAS_GPS
void MenuFunctions::displayGPSInfo() {
  ...
  display_obj.tft.println("--- Info GPS ---");
  display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
  display_obj.tft.print("Satelites: ");
  display_obj.tft.println(gps_obj.getNumSatsString());
  display_obj.tft.print("Latitude: ");
  display_obj.tft.println(gps_obj.getLat());
  display_obj.tft.print("Longitude: ");
  display_obj.tft.println(gps_obj.getLon());
  display_obj.tft.print("Altitude: ");
  display_obj.tft.println(gps_obj.getAlt());
}
#endif
