...
#ifdef HAS_SCREEN
#if defined(CYD_28)
  static const uint16_t calData_CYD_28[5] = {350, 3465, 188, 3431, 2};
#elif defined(CYD_24)
  static const uint16_t calData_CYD_24[5] = {481, 3053, 433, 3296, 3};
#elif defined(CYD_24CAP)
  static const uint16_t calData_CYD_24CAP[5] = {405, 3209, 297, 3314, 2};
#elif defined(CYD_24G)
  static const uint16_t calData_CYD_24CAP[5] = {405, 3209, 297, 3314, 2};
#elif defined(CYD_32)
  static const uint16_t calData_CYD_32[5] = {251, 3539, 331, 3534, 6};
#elif defined(CYD_35)
  static const uint16_t calData_CYD_35[5] = {309, 3465, 297, 3552, 6};
#elif defined(TFT_DIY)
  static const uint16_t calData_TFT_DIY[5] = {339, 3470, 237, 3438, 2};
#endif
...
void Display::tftDrawRedOnOffButton() {
  ...
  tft.drawString(text03, GREENBUTTON_X + (GREENBUTTON_W / 2),
                   GREENBUTTON_Y + (GREENBUTTON_H / 2));
  this->SwitchOn = false;
}
...
void Display::tftDrawGreenOnOffButton() {
  ...
  tft.setTextSize(2);
  tft.setTextDatum(MC_DATUM);
  tft.drawString(text04, REDBUTTON_X + (REDBUTTON_W / 2) + 1,
                   REDBUTTON_Y + (REDBUTTON_H / 2));
  this->SwitchOn = true;
}
...
void Display::tftDrawGraphObjects(byte x_scale) {
  ...
  tft.drawFastVLine(10, 5, HEIGHT_1 - 10, TFT_WHITE);  // y axis (adjusted for screen
                                                        // height)
...
void Display::tftDrawXScaleButtons(byte x_scale) {
  ...
  char button_minus0[] = "-";
  char button_plus0[] = "+";
  key[0].initButton(&tft, 220, 10, 20, 20, TFT_BLACK, TFT_CYAN, TFT_BLACK,
                   button_minus0, 2);
  key[1].initButton(&tft, 249, 10, 20, 20, TFT_BLACK, TFT_CYAN, TFT_BLACK,
                   button_plus0, 2);
  key[0].setLabelDatum(MC_DATUM, 1, 5);
  key[1].setLabelDatum(MC_DATUM, 1, 5);
  key[0].drawButton();
  key[1].drawButton();
}
...
void Display::tftDrawYScaleButtons(byte y_scale) {
  ...
  char button_minus1[] = "-";
  char button_plus1[] = "+";
  key[2].initButton(&tft, 276, 10, 20, 20, TFT_BLACK, TFT_MAGENTA, TFT_BLACK,
                   button_minus1, 2);
  key[3].initButton(&tft, 305, 10, 20, 20, TFT_BLACK, TFT_MAGENTA, TFT_BLACK,
                   button_plus1, 2);
  key[2].setLabelDatum(MC_DATUM, 1, 5);
  key[3].setLabelDatum(MC_DATUM, 1, 5);
  key[2].drawButton();
  key[3].drawButton();
}
...
void Display::tftDrawChannelScaleButtons(int set_channel, bool lnd_an) {
  ...
  if (lnd_an) {
    ...
    key[4].initButton(&tft, 164, 10, EXT_BUTTON_WIDTH, EXT_BUTTON_WIDTH, TFT_BLACK, TFT_BLUE,
                      TFT_BLACK, (char*)button_minus2, 2);
    key[5].initButton(&tft, 193, 10, EXT_BUTTON_WIDTH, EXT_BUTTON_WIDTH, TFT_BLACK, TFT_BLUE,
                      TFT_BLACK, (char*)button_plus2, 2);
  } else {
    key[4].initButton(&tft, (EXT_BUTTON_WIDTH / 2) * 6,
                      (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH - 1, EXT_BUTTON_WIDTH,
                      EXT_BUTTON_WIDTH, TFT_BLACK, TFT_BLUE, TFT_BLACK, button_minus2, 2);
    key[5].initButton(&tft, (EXT_BUTTON_WIDTH / 2) * 10,
                      (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH - 1, EXT_BUTTON_WIDTH,
                      EXT_BUTTON_WIDTH, TFT_BLACK, TFT_BLUE, TFT_BLACK, button_plus2, 2);
  }

  key[4].setLabelDatum(MC_DATUM, 1, 5);
  key[5].setLabelDatum(MC_DATUM, 1, 5);
...
void Display::tftDrawExitScaleButtons(bool lnd_an) {
  char button_exit[] = "X";
  if (lnd_an) {
    key[6].initButton(&tft, 137, 137, EXT_BUTTON_WIDTH, EXT_BUTTON_WIDTH, TFT_ORANGE, TFT_RED,
                      TFT_BLACK, (char*)button_exit, 1);
  } else {
    key[6].initButton(&tft, EXT_BUTTON_WIDTH / 2, (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH - 1,
                      EXT_BUTTON_WIDTH, EXT_BUTTON_WIDTH, TFT_ORANGE, TFT_RED, TFT_BLACK,
                      const_cast<char*>(button_exit), 1);
  }

  key[6].setLabelDatum(MC_DATUM, 1, 5);
  key[6].drawButton();
}
...
void Display::scrollScreenBuffer(bool down) {
  if (!down) {
    this->screen_buffer->erase(this->screen_buffer->begin());
  }
}
#endif
#endif
...
void Display::displayBuffer(bool do_clear) {
  if (!this->display_buffer->empty()) {
    int print_count = 1;
    while (!display_buffer->empty() && (print_count > 0)) {
      ...
      if ((display_buffer->size() > 0) && (!loading)) {
        ...
        tft.print(display_buffer->front());
...
#else
      xPos = 0;
      #define MAX_SCREEN_BUFFER 100  // Define MAX_SCREEN_BUFFER
      if (this->screen_buffer->size() >= MAX_SCREEN_BUFFER) {
        this->scrollScreenBuffer(true);
      }
      if (!display_buffer->empty()) {
        screen_buffer->push_back(display_buffer->front());
        display_buffer->erase(display_buffer->begin());
      }
      for (size_t i = 0; i < (*this->screen_buffer).size(); i++) {
...
int Display::scroll_line(uint32_t color) {
  int yTemp = yStart;
  if (!tteBar) {
    tft.fillRect(0, yStart, blank[(yStart - TOP_FIXED_AREA) / TEXT_HEIGHT],
                 TEXT_HEIGHT, color);
    yStart += TEXT_HEIGHT;
    if (yStart >= YMAX - BOT_FIXED_AREA)
      yStart = TOP_FIXED_AREA + (yStart - YMAX + BOT_FIXED_AREA);
  } else {
    tft.fillRect(0, yStart, blank[(yStart - TOP_FIXED_AREA_2) / TEXT_HEIGHT],
                 TEXT_HEIGHT, color);
    yStart += TEXT_HEIGHT;
    if (yStart >= YMAX - BOT_FIXED_AREA)
...
void Display::jpegRender(int xpos, int ypos) {
  ...
  int32_t min_w = minimum((int32_t)mcu_w, (int32_t)(max_x % mcu_w));
  int32_t min_h = minimum((int32_t)mcu_h, (int32_t)(max_y % mcu_h));
  int32_t win_w = mcu_w;
  int32_t win_h = mcu_h;
